 ...
 * Fully excludes dynamic peripherals (e.g. Audio DAC / headphones / Bluetooth).
 */
function collectStaticHardwareComponents() {
    ...
    if (typeof window !== 'undefined' && typeof navigator !== 'undefined') {
        ...
        // 1. Operating System (Normalized)
        components.os = getNormalizedOS();

        // 2. Hardware Topology
        components.cpuCores = nav.hardwareConcurrency || 0;
        components.touchPoints = nav.maxTouchPoints || 0;

        // 3. Physical Display / Screen
        var scr = getScreenHardwareProfile();
        components.screenWidth = scr.width;
        components.screenHeight = scr.height;
        components.colorDepth = scr.colorDepth;
        components.pixelDepth = scr.pixelDepth;
        components.devicePixelRatio = scr.devicePixelRatio;

        // 4. Timezone & Locale Constants (Physical machine clock & zone)
        try {
        ...
        // 5. Physical GPU & WebGL Architecture Parameters
        components.webglProfile = JSON.stringify(getWebGLHardwareProfile());

        // 6. OS Installed System Fonts
        components.installedFonts = getInstalledSystemFonts().sort().join(',');
